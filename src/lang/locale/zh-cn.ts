...
export default {
  ...
  options: {
    'custom-command': {
      ...
      name: '自定义命令',
      description:
        '自定义命令是在 Linter 完成格式化后运行的 Obsidian 命令。这意味着 Obsidian 命令会在 YAML 时间戳修改之后运行，因此它们可能会导致在下次运行 Linter 时触发 YAML 时间戳修改。一个 Obsidian 命令只能选择一次。**_注意，这目前仅适用于格式化当前文件_**',
      warning:
    ...
    'custom-replace': {
      ...
      warning:
        '如果您不知道正则表达式是什么，请谨慎使用。另外，请确保您不要在 iOS 移动设备上使用后行断言，由于该平台不支持，会导致格式化失败',
  ...
  rules: {
    ...
    'auto-correct-common-misspellings': {
      name: '自动更正常见的拼写错误',
      description:
        '通过常见拼写错误字典自动将错误拼写更正为正确拼写。有关自动更正单词的完整列表，请参阅 [auto-correct map](https://github.com/platers/obsidian-linter/tree/master/src/utils/auto-correct-misspellings.ts)',
      'ignore-words': {
        name: '忽略单词',
    ...
    'format-yaml-array': {
      name: '格式化 YAML 数组',
      description:
        '允许将常规 YAML 数组格式化为多行或单行，并允许部分数组(`tags`, `aliases`) 保留 Obsidian 原有的 YAML 格式。请注意，单字符串转换为单行，如果存在多个条目，则会变为单行数组。对于单字符串转换为多行，情况也是如此，只是它变成了多行数组',
      'alias-key': {
        name: '格式化 YAML aliases',
        description:
          '打开 YAML aliases 部分的格式设置。不应与规则`YAML 标题别名`同时启用，因为它们可能不会很好地协同工作，或者它们可能有不同的格式样式选择，从而导致意外结果'
      },
      'tag-key': {
        name: '格式化 YAML tags',
    ...
    'remove-space-before-or-after-characters': {
      ...
      'characters-to-remove-space-before': {
        name: '移除字符前的空格',
        description:
          '移除指定字符前的空格。 **注意，在字符列表中使用`{`或`}`会意外影响文件，因为它在程序后台的忽略语法中使用**'
      },
      'characters-to-remove-space-after': {
        name: '移除字符后的空格',
        description:
          '移除指定字符后的空格。 **注意，在字符列表中使用`{`或`}`会意外影响文件，因为它在程序后台的忽略语法中使用**'
      }
    },
    ...
    'remove-trailing-punctuation-in-heading': {
      name: '移除标题中的结尾标点符号',
      description:
        '从标题的末尾删除指定的标点符号，确保忽略[HTML 字符实体](https://en.wikipedia.org/wiki/List_of_XML_and_HTML_character_entity_references)末尾的分号',
    ...
    'space-between-chinese-japanese-or-korean-and-english-or-numbers': {
      ...
      description:
        '确保中文、日文或韩文和英文或数字由单个空格分隔. [参考链接](https://github.com/sparanoid/chinese-copywriting-guidelines)'
    },
    // strong-style.ts
    'strong-style': {
      name: '粗体样式',
    ...
    'yaml-timestamp': {
      ...
      'force-retention-of-create-value': {
        name: '强制保留创建日期值',
        description:
          '沿用 YAML Front-matter 中已有的创建日期，忽略文档元数据。对于文档元数据更改（比如复制文件）导致的创建时间更改非常有用'
      },
      'date-modified': {
        name: '修改日期',
      ...
      format: {
        name: '格式',
        description:
          'Moment.js 语法格式（详情设置见[Moment format options](https://momentjscom.readthedocs.io/en/latest/moment/04-displaying/01-format/)'
      }
    },
    // yaml-title-alias.ts
    'yaml-title-alias': {
