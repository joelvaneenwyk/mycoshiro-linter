/**
 * Useful regular expressions for linting and parsing
 */
...
// eslint-disable-next-line regexp/no-super-linear-backtracking
export const tableSeparator = /(\|? *:?-+:? *\|?)(\| *:?-+:? *\|?)*( |\t)*$/gm;
...
export const urlRegex =
  /(https?:\/\/(?:www\.|(?!www))[a-z\d][a-z\d-]+[a-z\d]\.[^\s`\]'"‘’“”>]{2,}|www\.[a-z\d][a-z\d-]+[a-z\d]\.[^\s`\]'"‘’“”>]{2,}|https?:\/\/(?:www\.|(?!www))[a-z\d]+\.[^\s`\]'"‘’“”>]{2,}|www\.[a-z\d]+\.[^\s`\]'"‘’“”>]{2,})/gi;
...
export const htmlEntitiesRegex = /&\S+;$/m;
...
export const templaterCommandRegex = /<%[\s\S]*?%>/g;
// checklist regex
    ...
    for (const link of linkMatches) {
      ...
      if (link.includes('|')) {
        ...
        const newLink =
          ...
          link.substring(startLinkTextPosition + 1, link.length - 2).trim() +
          ']]';
