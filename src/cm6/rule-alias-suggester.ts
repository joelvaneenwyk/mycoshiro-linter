...
export class RuleAliasSuggest extends EditorSuggest<ruleInfo> {
  ...
  onTrigger(cursor: EditorPosition, editor: Editor, _: TFile): EditorSuggestTriggerInfo | null {
    ...
    const onFrontmatterDisabledRulesLine =
      lineContents.startsWith(disableRulesKeyWithColon) ||
  ...
  getSuggestions(context: EditorSuggestContext): ruleInfo[] {
    ...
    const suggestions = this.ruleInfo.filter(
      (r: ruleInfo) => (r.name.contains(query) || r.alias.contains(query)) && !disabledRules.includes(r.alias)
    );
  ...
  selectSuggestion(suggestion: ruleInfo): void {
    if (this.context) {
      ...
      (this.context.editor as Editor).replaceRange(`${suggestedValue} `, this.context.start, this.context.end);
    }
  }
