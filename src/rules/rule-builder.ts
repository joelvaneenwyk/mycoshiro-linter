...
export abstract class RuleBuilderBase {
  ...
  static getRule<TOptions extends Options>(this: new () => RuleBuilder<TOptions>): Rule {
    if (!RuleBuilderBase.#ruleMap.has(this.name)) {
      ...
      const rule = new Rule(
        builder.nameKey,
        builder.descriptionKey,
        builder.settingsKey,
        builder.alias,
        builder.type,
        builder.safeApply.bind(builder),
        builder.exampleBuilders.map((b) => b.example),
        builder.optionBuilders.map((b) => b.option),
        builder.hasSpecialExecutionOrder,
        builder.ignoreTypes
      );
  ...
  static applyIfEnabledBase(
    rule: Rule,
    text: string,
    settings: LinterSettings,
    extraOptions: Options
  ): [result: string, isEnabled: boolean] {
  ...
  static hasSettingControl(optionsClassName: string, optionsClassKey: string) {
    return (
      !RuleBuilderBase.#noSettingsControlMap.has(optionsClassName) ||
      !RuleBuilderBase.#noSettingsControlMap.get(optionsClassName).includes(optionsClassKey)
    );
  }
}
...
export default abstract class RuleBuilder<TOptions extends Options> extends RuleBuilderBase {
  ...
  static register<TOptions extends Options>(
  ...
  static applyIfEnabled<TOptions extends Options>(
    this: typeof RuleBuilderBase & (new () => RuleBuilder<TOptions>),
    text: string,
    settings: LinterSettings,
  ...
  static getRuleOptions<TOptions extends Options>(
    this: new () => RuleBuilder<TOptions>,
    settings: LinterSettings
  ): TOptions {
...
export class ExampleBuilder<TOptions extends Options> {
  ...
  constructor(args: { description: string; before: string; after: string; options?: TOptions }) {
    this.example = new Example(args.description, args.before, args.after, args.options);
  }
}

type KeysOfObjectMatchingPropertyValueType<TObject, TValue> = {
  [TKey in keyof TObject]-?: TObject[TKey] extends TValue ? (TValue extends TObject[TKey] ? TKey : never) : never;
...
export class NumberOptionBuilder<TOptions extends Options> extends OptionBuilder<TOptions, number> {
  protected buildOption(): Option {
    return new TextOption(this.configKey, this.nameKey, this.descriptionKey, this.defaultValue.toString());
  }
}

export class DropdownOptionBuilder<TOptions extends Options, TValue extends string> extends OptionBuilder<
  TOptions,
  TValue
> {
  ...
  constructor(
    args: OptionBuilderConstructorArgs<TOptions, TValue> & {
      records: {
        value: TValue;
        description: string;
      }[];
    ...
    super(args);
    this.records = args.records.map(
      (record) => new DropdownRecord(('enums.' + record.value) as LanguageStringKey, record.description)
    );
  }
...
export class TextAreaOptionBuilder<TOptions extends Options> extends OptionBuilder<TOptions, string[]> {
  ...
  splitter: RegExp;
  constructor(
    args: OptionBuilderConstructorArgs<TOptions, string[]> & {
      separator?: string;
      splitter?: RegExp;
    }
  ) {
  ...
  setRuleOption(ruleOptions: TOptions, options: Options) {
    if (options[this.configKey] !== undefined) {
      ...
      const optionValue = (
        (options[this.configKey] as string).split(this.splitter) as TOptions[KeysOfObjectMatchingPropertyValueType<
          TOptions,
          string[]
        >]
...
export class MomentFormatOptionBuilder<TOptions extends Options> extends OptionBuilder<TOptions, string> {
  protected buildOption(): Option {
    return new MomentFormatOption(this.configKey, this.nameKey, this.descriptionKey, this.defaultValue.toString());
  }
}
