...
export default class MoveTagsToYaml extends RuleBuilder<MoveTagsToYamlOptions> {
  ...
  get exampleBuilders(): ExampleBuilder<MoveTagsToYamlOptions>[] {
    return [
      ...
      new ExampleBuilder({
        description:
          'Move tags from body to YAML with existing tags retains the already existing ones and only adds new ones',
        before: dedent`
          ---
          tags: [test, tag2]
          ---
      ...
      new ExampleBuilder({
        description:
          "Move tags to YAML frontmatter and then remove hashtags in body content tags when `Body tag operation = 'Remove hashtag'` and `Tags to ignore = 'yet-another-ignored-tag'`.",
        before: dedent`
          ---
          tags: [test, tag2]
          ---
      ...
      new ExampleBuilder({
        description:
          "Move tags to YAML frontmatter and then remove body content tags when `Body tag operation = 'Remove whole tag'`.",
        before: dedent`
          ---
          tags: [test, tag2]
          ---
