...
ruleTest({
  ...
  testCases: [
    ...
    {
      testName:
        'Make sure that a multiline array with a key value pair in it gets escaped if there are no subsequent key value pairs at the same indentation level',
      before: dedent`
        ---
        gists:
          - id: testing123
    ...
    {
      testName:
        'Make sure that a multiline array with a key value pair in it gets escaped if and does not throw an error if there are no further lines after it',
      before: dedent`
        ---
        gists:
          - id: testing123
